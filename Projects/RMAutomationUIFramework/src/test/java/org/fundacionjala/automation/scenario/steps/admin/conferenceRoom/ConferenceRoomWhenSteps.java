...
public class ConferenceRoomWhenSteps {
	...
	return new AdminPage().leftMenu.clickOnConferenceRoomsButton();
    }
	...
	rooms = goToConferenceRoomPage();
	while (message.equals("Room " + roomName + " was enabled")
		|| message.contains("Error on trying to")) {
	    message = rooms.clickOnTurnOnOffButton(roomName).getMessage();
	}
    }
	...
	home.leftMenu.clickOnConferenceRoomsButton().selectPageSize(pageSize);
    }
	...
	AdminPage home = new AdminPage();
	home.leftMenu.clickOnConferenceRoomsButton().selectOutOfOrderIcon(
		roomName);
    }
    ...
    public void i_create_an_Out_Of_Order_on_room_with_a_title_defined(
