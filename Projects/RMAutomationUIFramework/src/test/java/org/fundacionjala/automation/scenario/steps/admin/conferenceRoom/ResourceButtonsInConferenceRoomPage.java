...
public class ResourceButtonsInConferenceRoomPage {
	...
	Home.leftMenu.clickOnResourcesButton().clickOnAddButton()
		.setResourceName(resourceName).setDisplayName(resourceName)
		.setDescription(resourceName).clickOnSaveButton().refreshPage();
    }

    @Then("^I validate if the resource is in Conference Room page\\.$")
    public void i_validate_if_the_resource_is_in_Conference_Room_page()
	    throws Throwable {
    ...
    @After ("@conferenceResource")
    public void teardownConferenceRoomResource() throws InterruptedException {
    
	Home.leftMenu.clickOnResourcesButton().selectResource(expectedResult)
